...
const znl =
  "Aries (Mesha)~Taurus (Vrushaba)~Gemini (Mithuna)~Cancer (Karka)~Leo (Simha)~Virgo (Kanya)~Libra (Tula)~Scorpio (Vrushika)~Sagittarius (Dhanu)~Capricorn (Makara)~Aquarius (Kumbha)~Pisces (Meena)"; // Zodiac Map
const zsign = znl.split("~");
const range = [
  1, 31, 1, 12, 1800, 2100, 0, 23, 0, 59, 0, 0, 0, 12, 0, 59, 0, 179, 0, 59, 0,
  0, 0, 89, 0, 59,
];
const naks = [
  "Ashwini",
...
class AstroCore {
  ...
  static validateEntries(f) {
    if (f.birthDay && f.birthMonth && f.birthYear) {
      if (
        typeof f.birthDay === "number" &&
        typeof f.birthMonth === "number" &&
    ...
    if (f.birthHour && f.birthMinute) {
      if (
        typeof f.birthHour === "number" &&
        typeof f.birthMinute === "number"
      ) {
    ...
    if (f.birthZone) {
      if (typeof f.birthZone !== "number") {
        throw new Error(
          "Birth zone should be number. Eg: India (5.5) => +5:30 => 5.5, so input should be 5.5"
        );
      }
  ...
  static calculate(inputLunarCalc) {
    ...
    birthTimeZone = tz;
    let ln = inputLunarCalc.longitude;
    let la = inputLunarCalc.latitude;
    ...
    /* eslint max-len:"off" */
    dayOfWeek = AstroCore.calculateDayFromDate(
      +inputLunarCalc.birthDay,
      +inputLunarCalc.birthMonth,
      +inputLunarCalc.birthYear
    );
    // Moon mean longitude
    ...
    b +=
      0.001828 * Math.sin(4 * d - f - mm) -
      0.001803 * e * Math.sin(f + m) -
      0.00175 * Math.sin(3 * f) +
      0.00157 * e * Math.sin(mm - m - f) -
    ...
    r +=
      0.000401 * e * Math.cos(2 * d - m - mm) +
      0.00032 * e * Math.cos(mm - m) -
      0.000271 * Math.cos(d) -
    ...
    dlt = Math.atan2(
      c * Math.sin(d2r * h),
      r * Math.cos(d2r * dc) - c * Math.cos(d2r * h)
    );
    dlt *= r2d;
    hh = h + dlt;
    dlt /= 15;
    ...
    birthDasha = `${lord1.substr(nl * 3, 3)}/${lord1.substr(
      bk * 3,
      3
    )}/${lord1.substr(pd * 3, 3)}`;
  ...
  static calccurdasha(cd, nl) {
    ...
    const str = `${lord1.substr(nl * 3, 3)}/${lord1.substr(
      bk * 3,
      3
    )}/${lord1.substr(pd * 3, 3)}`;
    return str;
  ...
  static calculateDayFromDate(dd, mm, yyyy) {
    ...
    yyyy -= monVal;
    const day =
      (yyyy + yyyy / 4 - yyyy / 100 + yyyy / 400 + t[mm - 1] + dd) % 7;
   ...
   */
  static calcayan(t) {
    const ln = 125.044555 - 1934.1361849 * t + 0.0020762 * t * t; // Mean lunar node
    let off = 280.466449 + 36000.7698231 * t + 0.0003106 * t * t; // Mean Sun
    off =
  ...
  static ecl2equ(ln, la, ob) {
    let y = Math.sin(
      Math.sin(d2r * la) * Math.cos(d2r * ob) +
        Math.cos(d2r * la) * Math.sin(d2r * ob) * Math.sin(d2r * ln)
    );
    y =
      Math.sin(d2r * ln) * Math.cos(d2r * ob) -
      Math.tan(d2r * la) * Math.sin(d2r * ob);
  ...
  static equ2ecl(ra1, dc1, ob) {
    ra1 *= 15;
    let y =
      Math.sin(d2r * ra1) * Math.cos(d2r * ob) +
      Math.tan(d2r * dc1) * Math.sin(d2r * ob);
    ...
    pln = x;
    y = Math.sin(
      Math.sin(d2r * dc1) * Math.cos(d2r * ob) -
